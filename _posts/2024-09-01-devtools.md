...
Only the Console and Inspector are supported for now, while other valuable features like the Network and Storage tabs are still not functional.
...
We are working on **developer documentation** that will be available in the [Servo book](https://book.servo.org) to make future contributions easier.
